...
func (r *repository) CreateOrder(ctx context.Context, orders *models.Orders) (*models.Orders, error) {
	...
	if err := r.client.QueryRow(ctx, queryOneCreateOrder,
		&orders.OrderUID,
		&orders.TrackNumber,
		&orders.Entry,
		&orders.Locale,
...
func (r *repository) GetFullById(ctx context.Context, uid string) (*models.Orders, error) {
	...
	for rows.Next() {
		err = r.client.QueryRow(ctx, queryCreateItem, uid).Scan(
			&item.ChrtId,
			&item.TrackNumber,
			&item.Price,
			&item.Rid,
...
func NewRepository(client postgresql.Client) services.PGRepository {
	return &repository{
		client: client,
	}
}
